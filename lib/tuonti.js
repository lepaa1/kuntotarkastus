...
import { osioJarjestys } from '../data/tarkastuskohdat.js';
import * as db from './db.js';
import { lueZip } from './zip.js';
import { kuvatiedostonNimi } from './vienti.js';
...
export async function tuoKohde(esikatselu, tapa = 'korvaa') {
  ...
  for (const osioId of osioJarjestys(kohde)) {
    ...
    for (let i = 0; i < kuvat.length; i++) {
      ...
      delete kuvat[i].alkuperaAvain;
      delete kuvat[i].merkinnat;
      // Kameran taysikokoisia alkuperaisia ei viedä pakettiin.
      delete kuvat[i].kameraAvain;
    }
  }
