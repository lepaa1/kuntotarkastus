...
function osioSyote(kohde, osioId) {
  ...
  m.kohdat.forEach((teksti, i) => {
    ...
    // Tarkastuskohdan omat lähikuvat: määrä kertoo, mistä havainnosta on
    // kuvamateriaalia. Kuvatekstejä ei pyydetä näille erikseen, jottei
    // osion kuvatekstien numerointi mene sekaisin.
    const kohtakuvia = (o.kohtakuvat?.[i] || []).length;
    if (kohtakuvia) rivi.omia_kuvia = kohtakuvia;
    kohdat.push(rivi);
  });
